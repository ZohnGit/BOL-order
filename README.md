...
- 支持选择已保存的组合，也可以整对删除。
- 登录成功后显示“选择日期”，可设置起始日期和结束日期。
- 按原工作流分页读取 `FBB`、`ALL` 状态订单。
- 原工作流 `If` False 出口的整单取消订单显示在结果表中，并导出 SKU。
...
- 从第 1 页开始查询。
- 最后一条订单日期大于或等于起始日期时继续翻页。
- 商品净数量为 `quantity - quantityCancelled`。
- 所有商品行净数量都不大于 0 时，订单进入 `If` False 取消单出口。
- 取消单和正常订单都会按起始日期到结束日期的范围筛选，日期时间按 00:00 处理。
- 取消单会继续获取订单详情，并按订单商品导出 SKU；一个取消单多个 SKU 时会显示多行。
- 每单处理前等待 1.5 秒。
